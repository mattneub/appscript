...
from appscript import terminologyparser
# from appscript.keywordwrapper import Keyword
import osaterminology.defaultterminology

__all__ = ['kProperty', 'kElement', 'kCommand', 'Terminology']
...
class Terminology:
	...
	def defaulttables(self):
		if self._defaulttables is None:
			self._defaulttables = _maketypetable([], [], []) + _makereferencetable([], [], []) # (typebycode, typebyname, referencebycode, referencebyname)
		return self._defaulttables
	...
	def tablesforaetes(self, aetes):
		...
		"""
		classes, enums, properties, elements, commands = terminologyparser.buildtablesforaetes(aetes, self._style)
...
# t = Terminology('objc-appscript'); import aem, pprint; pprint.pprint( t.tablesforapp(aem.Application('/applications/textedit.app')) )
