...
# The ASSafeObject class undefines any methods not on the EXCLUDE safe list; traps are
# then applied to Module#method_added and Module#included to detect any subsequent
# method additions and immediately remove those from ASSafeObject as well.
...
# ASSafeObject after ASSafeObject is defined will show up in the
# list of available ASSafeObject methods. We handle this by defining
...
class Module
	...
	define_method(:included) do |mod|
		...
		if mod == Object
			public_instance_methods(true).each { |name| ASSafeObject.hide(name) }
			#private_instance_methods(true).each { |name| ASSafeObject.hide(name) }
			protected_instance_methods(true).each { |name| ASSafeObject.hide(name) }
		end
		return result
	end
end
