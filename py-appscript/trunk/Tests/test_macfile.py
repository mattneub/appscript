...
class TC_MacFile(unittest.TestCase):

	dir = '/private/tmp'
	
	def setUp(self):
		self.path1 = os.tempnam(self.dir, 'py-macfile-test.') # tempnam raises a security warning re. security; ignore it
		file(self.path1, 'w').close()
		fname = os.path.split(self.path1)[1]
		self.path2 = os.path.join(self.dir, 'moved-' + fname)
		# print "path: %r" % self.path1 # e.g. /private/tmp/py-macfile-test.VLrUW7
	
	def test_alias(self):
		# make alias
		self.f = macfile.Alias(self.path1)
		
		self.assertEqual("macfile.Alias(u%r)" % self.path1,  repr(self.f))
		
		#print "alias path 1: %s" % f.path # e.g. /private/tmp/py-macfile-test.VLrUW7
		...
		# print "alias path 2: %r" % f.path # /private/tmp/moved-py-macfile-test.VLrUW7
		self.assertEqual(self.path2, self.f.path)

		self.assertEqual("macfile.Alias(u%r)" % self.path2, repr(self.f))
	...
	def test_fileURL(self):
		...
		g = macfile.File('/non/existent path')
		...
		self.assertEqual("macfile.File(u'/non/existent path')", repr(g.file))
