...
try:
	...
	outFormat = osax.chooseitems([kRaw, kText, kAppscript, kAppleScript], 'Select output formats:', multiselelect=True)
	...
	for path in appPaths:
		...
		try:
			...
			if kAppscript in outFormat:
				htmldoc.doc(path, join(outFolder, splitext(basename(path))[0] + '_appscript.html') , 'appscript', options, userTemplateHTML)
			...
			print_exc()
			failedApps.append('%s (error: %s)' % (basename(path), err))
	if failedApps:
