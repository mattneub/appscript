...
# The RCASSafeObject class undefines any methods not on the EXCLUDE safe list; traps are
# then applied to Module#method_added and Module#included to detect any subsequent
# method additions and immediately remove those from RCASSafeObject as well.
...
require 'cocoa_appscript/reservedkeywords'

class RCASSafeObject

	EXCLUDE = RCAReservedKeywords + [
		"Array",
		"Float",
		"Integer",
		"String",
...
# RCASSafeObject after RCASSafeObject is defined will show up in the
# list of available RCASSafeObject methods. We handle this by defining
...
class Module
	...
	define_method(:included) do |mod|
		...
		if mod == Object
			public_instance_methods(true).each { |name| RCASSafeObject.hide(name) }
			#private_instance_methods(true).each { |name| RCASSafeObject.hide(name) }
			protected_instance_methods(true).each { |name| RCASSafeObject.hide(name) }
		end
		return result
	end
end
